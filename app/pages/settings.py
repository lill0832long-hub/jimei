...
def render_settings():
    ...
    with ui.card().classes("w-full"):
        ...
        with ui.tabs() as tabs:
            tab_ledger = ui.tab("账套管理", icon="folder")
            tab_account = ui.tab("科目管理", icon="list")
            tab_system = ui.tab("系统设置", icon="settings")

        with ui.tab_panels(tabs, value=tab_ledger).classes("w-full"):
            # ── Tab 1: 账套管理 + 期初余额 ──
                ...
                with ui.row().classes("gap-3"):
                    ...
                    with ui.card().classes("flex-1"):
                        with ui.card_section().classes("py-2 px-3 border-b border-grey-1"):
                            ui.label("📁 账套列表").classes("text-sm font-semibold")
                            ...
                            with ui.column().classes("gap-1 mt-1"):
                                ...
                                def _save_opening():
                                    try:
                                        ...
                                        do_set_opening_balance(lid, ob_code.value, _year, _month, ob_balance.value if ob_balance.value is not None else 0)
            ...
            # ── Tab 2: 科目管理 ──
            with ui.tab_panel(tab_account):
                with ui.card().classes("w-full"):
                    with ui.card_section().classes("py-2 px-3 border-b border-grey-1"):
                        ui.label("📚 添加新科目").classes("text-sm font-semibold")
                    with ui.card_section().classes("py-2 px-3"):
                        with ui.column().classes("gap-1 mt-1"):
                            with ui.row().classes("items-center gap-1"):
                                ui.label("科目代码").classes("text-sm")
                                ui.label("*").classes("text-red-500 text-sm")
                            ac_code = ui.input("科目代码", placeholder="100201").props("outlined dense").classes("w-full")
                            with ui.row().classes("items-center gap-1"):
                                ui.label("科目名称").classes("text-sm")
                                ui.label("*").classes("text-red-500 text-sm")
                            ...
                            def _do_add_acct():
                                ...
                                try:
                                    do_add_account(ac_code.value, ac_name.value, ac_cat.value)
                                finally:
                                    add_btn.props(remove="loading")

                            add_btn = ui.button("➕ 添加科目", color="blue", on_click=_do_add_acct).props("dense").classes("w-full")

            # ── Tab 3: 系统设置（公司信息 + 凭证字 + 操作日志 + 用户权限）──
            with ui.tab_panel(tab_system):
                ...
                with ui.card().classes("w-full"):
                    ...
                    with ui.card_section().classes("py-2 px-3"):
                        ...
                        def _save_company():
                            save_co_btn.props("loading")
                            try:
                                show_toast("公司信息已保存", "success")
                            finally:
                                save_co_btn.props(remove="loading")

                        save_co_btn = ui.button("💾 保存公司信息", color="blue", on_click=_save_company).props("dense").classes("mt-1")

                # 凭证字设置
                with ui.card().classes("w-full mt-3"):
                    with ui.card_section().classes("py-2 px-3 border-b border-grey-1"):
                        ui.label("📝 凭证字设置").classes("text-sm font-semibold")
                    with ui.card_section().classes("py-2 px-3"):
                        cols_vt = [
                ...
                with ui.card().classes("w-full mt-3"):
                    with ui.card_section().classes("py-2 px-3 border-b border-grey-1"):
                        ui.label("📋 最近操作日志").classes("text-sm font-semibold")
                    with ui.card_section().classes("py-2 px-3"):
                ...
                with ui.card().classes("w-full mt-3"):
                    with ui.card_section().classes("py-2 px-3 border-b border-grey-1"):
                        ui.label("👥 用户与权限").classes("text-sm font-semibold")
                    with ui.card_section().classes("py-2 px-3"):
                        ...
                        user_rows = [{**u, "role": role_labels.get(u["role"], u["role"]),
                                      "status": "启用" if u["is_active"] else "禁用"} for u in (users or [])]
...
def do_create_ledger(d, name, company):
    ...
    try:
        ...
        show_toast(f"❌ 创建失败: {e}", "error")


def do_set_opening_balance(lid, code, year, month, balance):
    _bal = balance if balance is not None else 0
    LedgerService.set_opening_balance(lid, code, year, month, _bal)
    acct_name = next((a["name"] for a in AccountService.get_all(lid) if a["code"] == code), code)
    show_toast(f"✅ {acct_name} 期初余额已设置：¥{_bal:,.2f}", "success")
    refresh_main()
