...
class AuthService:
    ...
    def create(username, password, role="user", ledger_id=None):
        ...
        return run_async(_auth_repo.create(
    ...
    def authenticate(username, password):
        user = run_async(_auth_repo.get_by_username(username=username))
        if user is None:
            return None
        # 检查用户是否激活
    ...
    def get_all(ledger_id=None):
        return to_dict(run_async(_auth_repo.get_all(ledger_id=ledger_id)))

    @staticmethod
    def update(user_id, **kwargs):
        return to_dict(run_async(_auth_repo.update(user_id, **kwargs)))

    @staticmethod
    def delete(user_id):
        return to_dict(run_async(_auth_repo.delete(user_id)))

    @staticmethod
    def change_password(user_id, new_password):
        password_hash = AuthService.hash_password(new_password)
        return to_dict(run_async(_auth_repo.update(user_id, password_hash=password_hash)))
    ...
    def get_audit_logs(ledger_id, limit=50, module=None, action=None, start_date=None, end_date=None):
        return to_dict(run_async(_audit_repo.get_logs(ledger_id, limit=limit, module=module, action=action)))
